  ...
  ("get_default_formation", [
     ...
     # (try_end),
      ...
      (else_try),
        ...
        (assign, reg0, formation_shield), #Khergit have underdeveloped infantry
      (else_try),
          ...
          (try_for_range, ":item_slot", ek_item_0, ek_head),
            ...
            (else_try),
              (try_begin),
                (neq, ":force_length", 0),
                (item_get_weapon_length, ":item_length", ":item"),
                (neg|item_has_property, ":item", itp_couchable), ### DAC Seek: Fix attempt for weapon switching loop
                (try_begin),
                  (lt, ":cur_score", ":item_length"),
                  (assign, ":cur_score", ":item_length"),
                  (assign, ":weapon", ":item"),
    ...
    (try_begin),
		...
		(else_try),	
			...
			(try_begin),
				...
				(try_begin),
                    ...
                    (item_has_property, ":wielded", itp_couchable),
	   ...
	   # actually have a lance equipped.  Otherwise this does nothing.
			...
			(else_try),
				...
				(try_begin),
					(is_between, ":wielded", ranged_weapons_begin, ranged_weapons_end), # Is it a bow or a horse-useful crossbow?
                    (neg|item_has_property, ":wielded", itp_cant_use_on_horseback),
	   ...
	   # actually have a bow equipped.  Otherwise this does nothing.
					(assign, ":end", ranged_weapons_end),  # adjust as needed
					(try_for_range, ":item", ranged_weapons_begin,":end"),
						(agent_has_item_equipped, ":agent", ":item"),
						(agent_set_wielded_item, ":agent", ":item"),
						(agent_set_slot, ":agent", slot_agent_horsebow, ":item"), #Mark horse archers for later use
						(agent_set_slot, ":agent", slot_agent_spear, 0), #double-check
						(assign, ":end", ranged_weapons_begin),#loop breaker      
					(try_end),
				(try_end),				
