...
#Notification menus
  ...
  (0,
	[
      ...
      (try_begin), # Piggyback loot distribution after battle on this one
	    (map_free),
        (troop_slot_ge, "trp_temp_troop", slot_troop_state, 1),
	    (call_script, "script_distribute_remaining_loot"),
      (try_end),
	]),
 ...
 #Increase castle food stores
  ...
  (14,
   [
	...
	(store_troop_gold, "$player_supply_budget", "trp_player"),
	(val_mul, "$player_supply_budget", 100),
    (assign, ":initial_supply_budget", "$player_supply_budget"),
    ...
    (try_for_range, ":unused", 0, ":consumption_amount"),
      ...
      (try_begin),
        ...
        (assign, ":counter", -1),
        (assign, ":selected_food_type", "itm_grain"),
        (try_for_range, ":cur_food", food_begin, food_end),
          (item_set_slot, ":cur_food", slot_item_is_checked, 0),
          (call_script, "script_cf_player_has_item_without_modifier", ":cur_food", imod_rotten),
          (val_add, ":counter", 1),
          (eq, ":counter", ":selected_food"),
          (assign, ":selected_food_type", ":cur_food"),
        (try_end),
        (call_script, "script_consume_food", ":selected_food", ":selected_food_type"),
      (else_try),
...
#NPC companion changes end
    ...
    (try_end),
    (try_begin),
	  (is_between, "$player_supply_base", centers_begin, centers_end),
	  (str_store_party_name, s1, "$player_supply_base"),
	  (try_begin),
	    (call_script, "script_cf_main_party_in_supply"),
        (store_sub, ":supply_cost", ":initial_supply_budget", "$player_supply_budget"),
        (call_script, "script_rand", 0, 100),
        (val_add, ":supply_cost", reg0), # Randomized rounding, to prevent gamey tricks
        (val_div, ":supply_cost", 100),
		(store_troop_gold, ":player_gold", "trp_player"),
	    (try_begin),
	      (le, ":supply_cost", 0),
	    (else_try),
	      (gt, ":player_gold", ":supply_cost"),
		  (assign, reg1, ":supply_cost"),
          (display_message, "@You received a shipment of supplies from {s1}."),
		  (troop_remove_gold, "trp_player", ":supply_cost"),
	    (else_try),
          (display_message, "@You can not afford to buy any more supplies from {s1}!", 0xFF8800),
	      (gt, ":player_gold", 0), # Pay what you can...
		  (troop_remove_gold, "trp_player", ":player_gold"),
	    (try_end),
	  (else_try),
        (display_message, "@You did not receive the expected supplies from {s1}!", 0xFF8800),
	  (try_end),
    (try_end),
    ]),
...
[
  ...
  # Paying for chartered ship
  (1, [
	(gt, "$players_captain", 0),
	(store_current_hours, ":time"),
	(troop_get_slot, ":last_payment", "$players_captain", slot_troop_last_talk_time),
	(val_sub, ":last_payment", ":time"),
	(le, ":last_payment", -24),
	(str_store_troop_name, s3, "$players_captain"),
	(troop_set_slot, "$players_captain", slot_troop_last_talk_time, ":time"),
    (store_troop_gold, ":gold", "trp_player"),
	(try_begin),
	  (ge, ":gold", ship_charter_cost),
      (display_message, "@You pay for {s3}'s crew."),
      (troop_remove_gold, "trp_player", ship_charter_cost),
    (else_try),
      (display_message, "@You can not afford to pay for {s3}'s crew.", 0xFF0000),
      (troop_remove_gold, "trp_player", ":gold"),
	  (store_sub, ":new_debt", ship_charter_cost, ":gold"),
	  (troop_get_slot, ":cur_debt", "$players_captain", slot_troop_player_debt),
	  (val_add, ":cur_debt", ":new_debt"),
	  (troop_set_slot, "$players_captain", slot_troop_player_debt, ":cur_debt"),
  ...
  (36,
   [
	   ...
	   (options_get_campaign_ai, ":difficulty_setting"), # On harder difficulties, spawn a few more bandits...
	   (call_script, "script_rand", 0, 2),
	   (le, ":difficulty_setting", reg0),
	   (call_script, "script_spawn_bandits"),
    ]),
